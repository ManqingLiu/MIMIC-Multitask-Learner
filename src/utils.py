...
class MimicData:
    def __init__(self, df, impute_outliers=True):
        self.df = df.copy()
        self.drop_cols = ['HADM_ID', 'SUBJECT_ID', 'HADMID_DAY', 'DOB', 'ADMITTIME','DEATHTIME']
        self.target = None
        if impute_outliers:
...
class MimicDataDeath(MimicData):
    def __init__(self, df):
        MimicData.__init__(self, df)
        self.target = 'DEATH'
        self.__append_death_outcome()

    def __append_death_outcome(self):
        self.df.loc[:, self.target] = (self.df['DEATH']).apply(lambda x: 1 if x == 1 else 0)
        print("Total death events:")
        print(self.df[self.target].value_counts())
        self.drop_cols = self.drop_cols + ['DEATHTIME']
